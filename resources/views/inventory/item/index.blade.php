	...
	<div class="panel-body">
		<table class="table table-striped table-hover table-condensed search-table">
			...
			<tbody>
				...
				<tr @if(Session::has('activeitem')) {{(Session::get('activeitem') == $value->id)?"class='info'":""}} @endif class="<?php if ($value->quantity() <= $value->min_level) echo 'highliht'; ?>">
					<td>{{ $value->name }}</td>
					<td>@if(!is_numeric($value->unit))
						{{$value->unit}}
						@else
						{{ $value->metrics->name }}
						@endif
					...
					<td>
						...
						<button class="btn btn-sm btn-danger delete-item-link" data-toggle="modal" data-target=".confirm-delete-modal" data-id="{{ route('item.delete', array($value->id)) }}">
