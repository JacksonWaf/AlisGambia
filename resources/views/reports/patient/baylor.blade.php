...
<body>
  <header>
    <table style="text-align:center;" border="0" width="100%"> 
      ...
      <tr>
        <td colspan="12" style="text-align:center;">
          @if(isset($tests))
            @if(!empty($tests->first()->revised_by))
            <b>{{trans('REVISED REPORT')}}</b>
            @elseif(!empty($tests->first()->approved_by))
  ...
  <table  border="0" width="100%"; style="border-bottom: 1px solid #cecfd5">
    <tr>
      ...
      <td width="30%">{{!is_null($patient->patient_number) ? $patient->patient_number : ''}}</td>
      <td width="20%"><b>Lab Accession No:</b></td>
    ...
    <tr>
      ...
      <td width="20%"><b>{{ trans('messages.gender')}} & DOB/{{ trans('messages.age')}}</b></td>
      <td width="30%">{{ $patient->getGender(false) }} | {{ $patient->dob}}/({{ $patient->getAge()}})</td>
    </tr>
    <tr>
      <td><b>Project/Collection site:</b></td>
      <td>@if(isset($tests))
    ...
    <tr>
      ...
      <td width="20%"><b>Date:</b>{{ ($tests->first()->time_created)?date_format(date_create($tests->first()->time_created), 'd-M-Y'):'' }}</td>
      <td width="30%"><b>Time:</b>{{ ($tests->first()->time_created)?date_format(date_create($tests->first()->time_created), 'H:i:s'):'' }}</td>
    </tr>
    <tr>
      <td width="20%"><b>Collected by</b></td>
      <td width="30%">@if(isset($tests))
                @if(!empty($tests->first()))
                    @if(!empty($tests->first()->specimen->accepted_by))
                        {{$tests->first()->specimen->acceptedBy->name}}
                    @endif
                  @endif
              @endif
      </td>
      <td width="20%"><b>Date:</b>{{ ($tests->first()->specimen->time_collected)?date_format(date_create($tests->first()->specimen->time_collected), 'd-M-Y'):'' }}</td>
      <td width="30%"><b>Time:</b>{{ ($tests->first()->specimen->time_collected)?date_format(date_create($tests->first()->specimen->time_collected), 'H:i:s'):'' }}</td>
    </tr>
    <tr>
      <td width="20%"><b>Picked by</b></td>
      <td width="30%">@if(isset($tests))
                @if(!empty($tests->first()))
                    @if(!empty($tests->first()->specimen->accepted_by))
                        {{$tests->first()->specimen->acceptedBy->name}}
      ...
      <td width="20%"><b>Date:</b>{{ ($tests->first()->specimen->time_accepted)?date_format(date_create($tests->first()->specimen->time_accepted), 'd-M-Y'):'' }}</td>
      <td width="30%"><b>Time:</b>{{ ($tests->first()->specimen->time_accepted)?date_format(date_create($tests->first()->specimen->time_accepted), 'H:i:s'):'' }}</td>
    </tr>
    <tr>
      <td width="20%"><b>Received by</b></td>
      <td width="30%">@if(isset($tests))
                @if(!empty($tests->first()))
                    @if($tests->first()->tested_by !=0)
                     {{ $tests->first()->testedBy->name }}
                     @else
                     ''
                    @endif
                  @endif
              ...
              <tr>
                ...
                <td>
                  @if($result->revised_result!=null)
                      {{ htmlspecialchars($result->revised_result)}}
                      @else
     ...
     @elseif($test->testType->isCulture())
       ...
       @foreach($test->isolatedOrganisms as $isolated_organism)
          <table style="border-bottom: 1px solid #cecfd5;" width="100%">
            <tr>
              <td rowspan="{{$isolated_organism->drugSusceptibilities->count()}}" class="organism"  width="40%"><i><b>{{$isolated_organism->organism->name}}</i></b></td>
                    <?php $i = 1; ?>
...
<table style="border-bottom: 0px solid #cecfd5; font-size:8px;font-family: 'Courier New',Courier;">
  ...
  <tr>
    <td>
      <strong>Approved By : <img src="{{public_path('/i/bonny.png')}}" class="manager" style="margin-top:0px; padding-right: 20px;" width="10%"></strong>
      <!-- <div style="margin-top:10px; padding-right: 20px;" width="10%"><img src="{{public_path('/i/bonny.png')}}" class="manager" style="margin-top:0px; padding-right: 20px;" width="10%"></div> -->
    </td>
  </tr>
    ...
    <table style="width: 100%;">
      <tr>
        <td style="text-align: right;">
              ...
              <img src="{{public_path('/i/baylor_stump.png')}}" class="manager" style="margin-top:-60px; padding-right: 20px;" width="20%">
              <div class="stamp-date" style="margin-top:-50px; padding-right: 65px;"><b style="color:red">{{date_format(date_create(now()), 'd M Y')}}</b></div>
