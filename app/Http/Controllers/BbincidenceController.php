...
class BbincidenceController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		if (Auth::user()->can('manage_national_biorisk')) {
			if ($datefrom != '') {
				...
				$bbincidences = Bbincidence::facility_search($search)->orderBy('id', 'DESC')->paginate(config('kblis.page-items'))->appends($request->except('_token'));
		} else {
	...
	public function create()
	{
		...
		//$facilitiesList = DB::table('facilities')->orderBy('name')->pluck('name', 'id')->toArray();

		//$userfacilityList = DB::table('facilities')->where('id', '=', Auth::user()->facility_id)->orderBy('name')->pluck('name', 'id')->toArray();

		//$naturesList = DB::table('unhls_bbnatures')->orderBy('priority')->orderBy('class')->pluck('name', 'id')->toArray();
	...
	public function store(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			if ($request->get('nature') != '') {
				$bbincidence->occurrence = implode(',', $request->get('nature'));
			}
	...
	public function analysisupdate(Request $request, $id)
	{
		...
		if ($validator->fails()) {
			...
			if ($request->get('cause') != '') {
				$bbincidence->cause = implode(',', $request->get('cause'));
			}
			$actions = $request->get('corrective_action');
			if ($request->get('corrective_action') != '') {
				$bbincidence->corrective_action = implode(',', $request->get('corrective_action'));
			}
