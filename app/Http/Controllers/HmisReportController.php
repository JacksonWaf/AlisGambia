...
class HmisReportController extends Controller {
	...
	public function hmis105(Request $request, $month = '')
	{
...
WHERE `v`.`created_at` LIKE '%".$month."%' GROUP BY s.id";
// GROUP BY t.specimen_id";
	...
	private function getHIVTestCounts($lab_section_id, $month){
		...
		LEFT JOIN measure_name_mappings mnm ON(utr.measure_id = mnm.measure_id)
		WHERE `ut`.`time_created` LIKE '%".$month."%' AND tt.name LIKE '%hiv%'
		GROUP BY ut.purpose, ut.test_type_id, mnm.system_name";
	...
	private function getAlphanumericTestCountsByLabSection($lab_section_id, $month){
		$query = "select ut.test_type_id, tt.name, utr.measure_id, mnm.measure_id as M2, mnm.system_name, tt.test_category_id as lab_section, count(DISTINCT ut.id) as total, SUM(case when mr.interpretation = 'Positive' OR utr.result LIKE '%Pos%' then 1 else 0 end) as Positive, 
SUM(case when mr.interpretation LIKE 'Negative' OR utr.result LIKE '%Neg%' then 1 else 0 end) as Negative FROM unhls_tests ut 
INNER JOIN test_types tt ON(ut.test_type_id = tt.id) 
INNER JOIN test_categories tc ON(tc.id = tt.test_category_id) 
INNER JOIN unhls_test_results utr ON(utr.test_id = ut.id) 
INNER JOIN testtype_measures ttm ON(ttm.measure_id = utr.measure_id) 
LEFT JOIN measure_ranges mr ON(mr.measure_id = utr.measure_id AND mr.alphanumeric = utr.result)
		LEFT JOIN measure_name_mappings mnm ON(utr.measure_id = mnm.measure_id)
		WHERE `ut`.`time_created` LIKE '%".$month."%' AND ut.test_status_id != 3 AND ut.test_status_id != 6
	...
	private function getHMIS108service($month){
		$query = "select blood_component, COUNT(bb.id) as recieved, SUM(CASE WHEN bb.status = 1 THEN 1 ELSE 0 END) as transfused FROM blood_bank bb LEFT JOIN patient_transfused pt ON(pt.blood_bank_id = bb.id)
	...
	private function getHCTCounts($month){
		...
		$test_type_counts['oraquick'] = 0;$test_type_counts['oraquick_h'] = 0;$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'hct'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_h'] = $row->hct;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'hct'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_h'] = $row->hct;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'hct'){
	 			$test_type_counts['sdbioline'] = $row->total;
	 			$test_type_counts['sdbioline_h'] = $row->hct;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'hct'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_h'] = $row->hct;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'hct'){
	...
	private function geteMTCTCounts($month){
		...
		$test_type_counts['sdbioline_p'] = 0;$test_type_counts['hiv_syphilis_duo'] = 0;$test_type_counts['hiv_syphilis_duo_p'] = 0;$test_type_counts['oraquick'] = 0;$test_type_counts['oraquick_p'] = 0;$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'pmtct'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_p'] = $row->pmtct;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'pmtct'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_p'] = $row->pmtct;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'pmtct'){
	 			$test_type_counts['sdbioline'] = $row->total;
	 			$test_type_counts['sdbioline_p'] = $row->pmtct;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'pmtct'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_p'] = $row->pmtct;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'pmtct'){
	 			$test_type_counts['oraquick'] = $row->total;
	...
	private function getclinicalDiagnosisCounts($month){
		...
		$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'clinical_diagnosis'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_cd'] = $row->clinical_diagnosis;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'clinical_diagnosis'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_cd'] = $row->clinical_diagnosis;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'clinical_diagnosis'){
	 			$test_type_counts['sdbioline'] = $row->total;
	 			$test_type_counts['sdbioline_cd'] = $row->clinical_diagnosis;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'clinical_diagnosis'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_cd'] = $row->clinical_diagnosis;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'clinical_diagnosis'){
	 			$test_type_counts['oraquick'] = $row->total;
	...
	private function getSMCCounts($month){
		...
		$test_type_counts['hiv_syphilis_duo_s'] = 0;$test_type_counts['oraquick'] = 0;$test_type_counts['oraquick_s'] = 0;$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'smc'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_s'] = $row->smc;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'smc'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_s'] = $row->smc;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'smc'){
	 			$test_type_counts['sdbioline'] = $row->total; $test_type_counts['sdbioline_s'] = $row->smc;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'smc'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_s'] = $row->smc;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'smc'){
	 			$test_type_counts['oraquick'] = $row->total;
	...
	private function getrepeatTestCounts($month){
		...
		$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'repeat_test'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_rt'] = $row->repeat_test;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'repeat_test'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_rt'] = $row->repeat_test;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'repeat_test'){
	 			$test_type_counts['sdbioline'] = $row->total;
	 			$test_type_counts['sdbioline_rt'] = $row->repeat_test;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'repeat_test'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_rt'] = $row->repeat_test;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'repeat_test'){
	 			$test_type_counts['oraquick'] = $row->total;
	...
	private function getVerificationTestCounts($month){
		...
		$test_type_counts['sdbioline_tv'] = 0;$test_type_counts['hiv_syphilis_duo'] = 0;$test_type_counts['hiv_syphilis_duo_tv'] = 0;$test_type_counts['oraquick'] = 0;$test_type_counts['oraquick_tv'] = 0;$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'test_for_verification'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_tv'] = $row->test_for_verification;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'test_for_verification'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_tv'] = $row->test_for_verification;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'test_for_verification'){
	 			$test_type_counts['sdbioline'] = $row->total;
	 			$test_type_counts['sdbioline_tv'] = $row->test_for_verification;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'test_for_verification'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_tv'] = $row->test_for_verification;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'test_for_verification'){
	 			$test_type_counts['oraquick'] = $row->total;
	...
	private function getinconclusiveCounts($month){
		...
		$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'inconclusive_result'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_ir'] = $row->inconclusive_result;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'inconclusive_result'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_ir'] = $row->inconclusive_result;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'inconclusive_result'){
	 			$test_type_counts['sdbioline'] = $row->total;
	 			$test_type_counts['sdbioline_ir'] = $row->inconclusive_result;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'inconclusive_result'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_ir'] = $row->inconclusive_result;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'inconclusive_result'){
	 			$test_type_counts['oraquick'] = $row->total;
	...
	private function getdnaConfirmedTestCounts($month){
		...
		$test_type_counts['sdbioline_dct'] = 0;$test_type_counts['hiv_syphilis_duo'] = 0;$test_type_counts['hiv_syphilis_duo_dct'] = 0;$test_type_counts['oraquick'] = 0;$test_type_counts['oraquick_dct'] = 0;$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'dna_confirmed_test'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_dct'] = $row->dna_confirmed_test;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'dna_confirmed_test'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_dct'] = $row->dna_confirmed_test;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'dna_confirmed_test'){
	 			$test_type_counts['sdbioline'] = $row->total;
	 			$test_type_counts['sdbioline_dct'] = $row->dna_confirmed_test;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'dna_confirmed_test'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_dct'] = $row->dna_confirmed_test;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'dna_confirmed_test'){
	 			$test_type_counts['oraquick'] = $row->total;
	...
	private function getIQCCounts($month){
		...
		$test_type_counts['sdbioline_qc'] = 0;$test_type_counts['hiv_syphilis_duo'] = 0;$test_type_counts['hiv_syphilis_duo_qc'] = 0;$test_type_counts['oraquick'] = 0;$test_type_counts['oraquick_qc'] = 0;$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'qc'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_qc'] = $row->qc;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'qc'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_qc'] = $row->qc;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'qc'){
	 			$test_type_counts['sdbioline'] = $row->total;
	 			$test_type_counts['sdbioline_qc'] = $row->qc;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'qc'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_qc'] = $row->qc;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'qc'){
	 			$test_type_counts['oraquick'] = $row->total;
	...
	private function getEQACounts($month){
		...
		$test_type_counts['sdbioline_eqa'] = 0;$test_type_counts['hiv_syphilis_duo'] = 0;$test_type_counts['hiv_syphilis_duo_eqa'] = 0;$test_type_counts['oraquick'] = 0;$test_type_counts['oraquick_eqa'] = 0;$test_type_counts['others'] = 0;
	 	...
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->system_name == $test_type_ids['determine'] && $row->purpose == 'eqa'){
	 			$test_type_counts['determine'] = $row->total;
	 			$test_type_counts['determine_eqa'] = $row->eqa;
	 		}else if($row->system_name == $test_type_ids['statpak'] && $row->purpose == 'eqa'){
	 			$test_type_counts['statpak'] = $row->total;
	 			$test_type_counts['statpak_eqa'] = $row->eqa;
	 		}else if($row->system_name == $test_type_ids['sdbioline'] && $row->purpose == 'eqa'){
	 			$test_type_counts['sdbioline'] = $row->total;
	 			$test_type_counts['sdbioline_eqa'] = $row->eqa;
	 		}else if($row->system_name == $test_type_ids['hiv_syphilis_duo'] && $row->purpose == 'eqa'){
	 			$test_type_counts['hiv_syphilis_duo'] = $row->total;
	 			$test_type_counts['hiv_syphilis_duo_eqa'] = $row->eqa;
	 		}else if($row->system_name == $test_type_ids['oraquick'] && $row->purpose == 'eqa'){
	 			$test_type_counts['oraquick'] = $row->total;
	...
	private function getHematologyCounts($month){
		...
		$test_type_counts['others'] = 0;
		$test_of_interest = array('hb_non_automated','cbc','film_comment','esr','bleeding_time','prothrombin_time','clotting_time','sickle_cell');
		
	 	foreach($rows as $row){
	 		//dd($row);
	 	// if(in_array($row->system_name, $test_of_interest)){
	 		...
	 		$test_type_counts['others'] = $test_type_counts['others'];
	...
	private function getSerologyCounts($month){
		...
		$test_type_counts['rpr'] = 0;
		$test_type_counts['rpr1'] = 0;
		...
		$test_type_counts['hcg'] = 0;
		$test_type_counts['hcg1'] = 0;
		...
		$test_type_counts['other'] = 0;
	 	...
	 	foreach($rows as $row){
	 		...
	 		if($row->system_name == $test_type_ids['vdrl_rpr']){
	 		...
	 		}else if($row->system_name == $test_type_ids['rpr']){
	 			$test_type_counts['rpr'] = $row->total;
	 			$test_type_counts['rpr1'] = $row->Positive;
	 		...
	 		}else if($row->system_name == $test_type_ids['hcg']){
	 			$test_type_counts['hcg'] = $row->total;
	 			$test_type_counts['hcg1'] = $row->Positive;
	 			...
	 			$test_type_counts['other'] = $test_type_counts['other'];
	 		}
		...
		$test_type_counts['other_parasites'] = 0;
	 	foreach($rows as $row){
	 		...
	 		if($row->system_name == $test_type_ids['entamoeba']){
	 			...
	 			$test_type_counts['other_parasites'] = $test_type_counts['other_parasites'];
	 		}
	...
	private function getCultureAndSensitivityCounts($month){
		...
		$query = "SELECT st.id, st.name, count(specimen_id) as total, test_status_id FROM unhls_tests ut JOIN specimens s on(s.id = ut.specimen_id)
JOIN specimen_types st on(st.id = s.specimen_type_id) JOIN test_name_mappings tnm on(ut.test_type_id = tnm.test_type_id) WHERE time_completed LIKE '%".$month."%' AND tnm.system_name = 'culture_sensitivity' Group by st.name";
		//set variables for the target tests -from
		//also get array of test_type ids to use in condition
	 	$rows = DB::select($query);
	 	foreach($rows as $row){
	 		//dd($row);
	 		if(stripos($row->name, $test_type_ids['blood'])){
	 			$test_type_counts['blood'] = $row->total;
	 		}else if($row->name = $test_type_ids['urine']){
	 			$test_type_counts['urine'] = $row->total;
	 		}else if($row->name = $test_type_ids['stool']){
	 			$test_type_counts['stool'] = $row->total;
	 		}else if($row->name = $test_type_ids['sputum']){
	 			$test_type_counts['sputum'] = $row->total;
	 		}else if($row->name = $test_type_ids['nosal_swab']){
	 			$test_type_counts['nosal_swab'] = $row->total;
	 		}else if($row->name = $test_type_ids['rectal_swab']){
	 			$test_type_counts['rectal_swab'] = $row->total;
	 		}else if($row->name = $test_type_ids['wound_swab']){
	 			$test_type_counts['wound_swab'] = $row->total;
	 		}else if($row->name = $test_type_ids['pus_swab']){
	 			$test_type_counts['pus_swab'] = $row->total;
	 		}else if($row->name = $test_type_ids['eye_swab']){
	 			$test_type_counts['eye_swab'] = $row->total;
	 		}else if($row->name = $test_type_ids['ear_swab']){
	 			$test_type_counts['ear_swab'] = $row->total;
	 		}else if($row->name = $test_type_ids['throat_swab']){
	 			$test_type_counts['throat_swab'] = $row->total;
	 		}else if($row->name = $test_type_ids['uretheral_swab']){
	 			$test_type_counts['uretheral_swab'] = $row->total;
	 		}
	...
	private function getMolecularCounts($month){
		...
		$rows = $this->getAlphanumericTestCountsByLabSection($test_type_ids['test_category_id'],$month);
	 	foreach($rows as $row){
	 		if($row->system_name == $test_type_ids['tb_genexpert_mtb']){
	 			$test_type_counts['tb_genexpert_mtb'] = $row->total;
	 			$test_type_counts['tb_genexpert_mtb1'] = $row->Positive;
	 		}else if($row->system_name == $test_type_ids['tb_genexpert_rr']){
	 			$test_type_counts['tb_genexpert_rr'] = $row->total;
	 			$test_type_counts['tb_genexpert_rr1'] = $row->Positive;
	 		}else if($row->system_name == $test_type_ids['latent_tb']){
	 			$test_type_counts['latent_tb'] = $row->total;
	 			$test_type_counts['latent_tb1'] = $row->Positive;
	 		}else if($row->system_name == $test_type_ids['tb_lam']){
	 			$test_type_counts['tb_lam'] = $row->total;
	 			$test_type_counts['tb_lam1'] = $row->Positive;
	 		}
	 	}
	 	return $test_type_counts;
	}
		...
		$test_type_counts['Other_reasons'] = 0;
	 	foreach($rows as $row){
	 		...
	 		if((int)$row->test_type_id == $test_type_ids['eid']){
	 			...
	 			$test_type_counts['Other_reasons'] = $test_type_counts['Other_reasons'];
	 		}
	...
	private function getTestTypeIDs($lab_section){
		...
		$test_type_ids = [
						...
						'molecular' => [
							'test_category_id' => 2,
							'tb_genexpert_mtb' => 'mtb',
							'tb_genexpert_rr' => 'rif_resistance',
							'latent_tb' => 'latent_tb',
							'tb_lam' => 'tb_lam',
						],
						'hctcount' => [
	...
	private function getTestTypeInitialCounts($lab_section){
		...
		//molecular
		$test_types['molecular']['tb_genexpert_mtb'] = 0;
		$test_types['molecular']['tb_genexpert_rr'] = 0;
		$test_types['molecular']['tb_genexpert_mtb1'] = 0;
		$test_types['molecular']['tb_genexpert_rr1'] = 0;
