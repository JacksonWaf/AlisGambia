	...
	public function store(Request $request)
	{
		...
		if ($validator->fails()) {
            ...
            if(is_array($testLists)){
                foreach ($testLists as $testList) {
                    ...
                    foreach ($testList['test_type_id'] as $id) {
                        ...
                        $test->requested_by = (!empty($request->get('requested_by'))) ? $request->get('requested_by') : 1 ;
			...
			//$url = Session::get('SOURCE_URL');
             return redirect()->route('unhls_test.index')->with('message', 'messages.success-creating-test')
