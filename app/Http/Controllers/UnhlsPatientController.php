...
class UnhlsPatientController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // $districtid = Auth::user()->facility->district_id;
        // dd($districtid);
        $facilityid = Auth::user()->facility->id;
        if ($search == null || $search == '') {

            $patients = UnhlsPatient::select(\DB::raw('MAX(unhls_patients.id) as id'), \DB::raw('MAX(unhls_patients.ulin) as ulin'), 'unhls_patients.patient_number', 'unhls_patients.name', 'unhls_patients.dob', 'unhls_patients.gender', 'unhls_patients.village_residence')
                ->where('hubid', $facilityid)
                ->groupBy('unhls_patients.name')
                // ->having('unhls_patients.name', '>=', '1')
                ->orderBy('id', 'desc')
                ->paginate(config('kblis.page-items'))
                ->appends($request->except('_token'));
        } else {
            $patients = UnhlsPatient::search($search)->select(\DB::raw('MAX(unhls_patients.id) as id'), \DB::raw('MAX(unhls_patients.ulin) as ulin'), 'unhls_patients.patient_number', 'unhls_patients.name', 'unhls_patients.dob', 'unhls_patients.gender', 'unhls_patients.village_residence')
                ->groupBy('unhls_patients.name')
                // ->having('unhls_patients.name', '>=', '1')
                ->orderBy('id', 'desc')
                ->paginate(config('kblis.page-items'))
    ...
    public function store(Request $request)
    {
        ...
        $nation = ['0' => 'Gambian', '1' => 'Senegalese', '2' => 'Guinean', '3' => 'Other'];
        // store
        ...
        $patient->hubid = Auth::user()->facility->id;
