...
use App\Exports\AlisRestrackExport;
...
class ReportController extends Controller
{
    ...
    public function loadPatients(Request $request)
    {
        ...
        $patients = UnhlsPatient::where('hubid', Auth::user()->facility->id)->paginate(20);
    ...
    public function loadPatientss(Request $request)
    {
        ...
        if ($search == null || $search == '') {
            $patients = UnhlsPatient::where('hubid', Auth::user()->facility->id)->orderBy('id', 'DESC')->paginate(config('kblis.page-items'))->appends($request->except('_token'));
        } else {
        ...
        $visits = UnhlsVisit::search($search)
            ->leftjoin('unhls_patients', 'unhls_patients.id', '=', 'unhls_visits.patient_id')->where('unhls_patients.hubid', '=', Auth::user()->facility->id)
            ->select('unhls_visits.id', 'unhls_visits.patient_id', 'unhls_visits.visit_type', 'unhls_visits.visit_number', 'unhls_visits.visit_lab_number', 'unhls_visits.facility_id',  'unhls_visits.facility_lab_number', 'unhls_visits.is_printed', 'unhls_visits.printed_by', 'unhls_visits.time_printed', 'unhls_visits.created_at', 'unhls_visits.updated_at', 'unhls_visits.ward_id', 'unhls_visits.bed_no', 'unhls_visits.visit_status_id', 'unhls_visits.hospitalized', 'unhls_visits.urgency', 'unhls_visits.on_antibiotics')->orderBy('id', 'DESC')->paginate(25)->appends($request->except('_token'));
    ...
    public function viewRevisedPatientReport(Request $request, $id, $visit = null, $testId = null)
    {
        ...
        $template = config('kblis.facility-report');
    ...
    public function viewFinalPatientReport(Request $request, $id, $visit = null, $testId = null)
    {
        ...
        $content = view($template)
            ...
            ->with('accredited', $accredited)
            ->with('verified', $verified)
            ->with('facilityname', $facilityname)
            ->withInput($request->all());
    ...
    public function viewInterimPatientReport(Request $request, $id, $visit = null, $testId = null)
    {
        ...
        $facility = DB::table('unhls_patients')->leftjoin('unhls_facilities', 'unhls_patients.hubid', '=', 'unhls_facilities.id')->where('unhls_patients.id', $id)->select('unhls_facilities.name', 'unhls_facilities.image_stamp')->get();
        $facilityname = $facility[0]->name;
        $facilitystamp = $facility[0]->image_stamp;
        // dd($facility);
        ...
        $content = view($template)
            ...
            ->with('verified', $verified)
            ->with('facilityname', $facilityname)
            ->with('facilitystamp', $facilitystamp)
            ->withInput($request->all());
    ...
    public function viewPatientVisitRequestForm(Request $request, $visit_id)
    {
        ...
        // adhoc config decision
        //$template = AdhocConfig::where('name','Report')->first()->getReportTemplate();
        $template = config('kblis.facility-requestForm');
        ...
        if ($records == 'patients') {
            if ($from || $to) {
                if (strtotime($from) > strtotime($to) || strtotime($from) > strtotime($date) || strtotime($to) > strtotime($date)) {
                    ...
                    $visits = UnhlsVisit::whereBetween('unhls_visits.created_at', array($from, $toPlusOne))
                        ->leftjoin('unhls_patients', 'unhls_patients.id', '=', 'unhls_visits.patient_id')->where('unhls_patients.hubid', '=', $facility_id)
                        ->select('unhls_visits.id', 'unhls_visits.patient_id', 'unhls_visits.visit_type', 'unhls_visits.visit_number', 'unhls_visits.visit_lab_number', 'unhls_visits.facility_id', 'unhls_visits.facility_lab_number', 'unhls_visits.is_printed', 'unhls_visits.printed_by', 'unhls_visits.time_printed', 'unhls_visits.created_at', 'unhls_visits.updated_at', 'unhls_visits.ward_id', 'unhls_visits.bed_no', 'unhls_visits.visit_status_id', 'unhls_visits.hospitalized', 'unhls_visits.urgency', 'unhls_visits.on_antibiotics')
                        ->get();
                    $male = UnhlsVisit::join('unhls_patients', 'unhls_visits.patient_id', '=', 'unhls_patients.id')->where('unhls_patients.hubid', '=', $facility_id)->where('gender', '=', 0)->whereBetween('unhls_visits.created_at', array($from, $toPlusOne))->get();
                }
                if (count($visits) == 0) {
                    Session::flash('message', trans('messages.no-match'));
                }
                ...
                $visits = UnhlsVisit::where('unhls_visits.created_at', 'LIKE', $date . '%')->orderBy('unhls_visits.patient_id')
                    ->leftjoin('unhls_patients', 'unhls_patients.id', '=', 'unhls_visits.patient_id')->where('unhls_patients.hubid', '=', Auth::user()->facility->id)
                    ->select('unhls_visits.id', 'unhls_visits.patient_id', 'unhls_visits.visit_type', 'unhls_visits.visit_number', 'unhls_visits.visit_lab_number', 'unhls_visits.facility_id', 'unhls_visits.facility_lab_number', 'unhls_visits.is_printed', 'unhls_visits.printed_by', 'unhls_visits.time_printed', 'unhls_visits.created_at', 'unhls_visits.updated_at', 'unhls_visits.ward_id', 'unhls_visits.bed_no', 'unhls_visits.visit_status_id', 'unhls_visits.hospitalized', 'unhls_visits.urgency', 'unhls_visits.on_antibiotics')
                    ->get();
                $male = UnhlsVisit::join('unhls_patients', 'unhls_visits.patient_id', '=', 'unhls_patients.id')->where('unhls_patients.hubid', '=', $facility_id)->where('gender', '=', 0)->whereBetween('unhls_visits.created_at', array($from, $toPlusOne))->get();
        ...
        //Rejected tests
            ...
            if ($from || $to) {
                if (strtotime($from) > strtotime($to) || strtotime($from) > strtotime($date) || strtotime($to) > strtotime($date)) {
                    ...
                    $tests = UnhlsTest::whereIn('test_status_id', [UnhlsTest::COMPLETED, UnhlsTest::VERIFIED, UnhlsTest::APPROVED])->whereBetween('time_created', array($from, $toPlusOne))->get(array('unhls_tests.*'));
                    $testcount = $tests->count();
                    $specimencount = $test_specimen->count();
                    $totaltestcounts = $testcount + $specimencount;
                    $percentagerate = $specimencount / $totaltestcounts;
                }
            } else {
                $test_specimen = $test_specimen->where('time_created', 'LIKE', $date . '%')->get(array('unhls_tests.*'));
                $tests = UnhlsTest::whereIn('test_status_id', [UnhlsTest::COMPLETED, UnhlsTest::VERIFIED, UnhlsTest::APPROVED])->where('time_created', 'LIKE', $date . '%')->get(array('unhls_tests.*'));
                $testcount = $tests->count();
                $specimencount = $test_specimen->count();
                $totaltestcounts = $testcount + $specimencount;
                $percentagerate = $specimencount / $totaltestcounts;
            ...
            if ($request->has('word')) {
                ...
                return view('reports.daily.specimen')
                    ...
                    ->with('testType', $testType)
                    ->with('accredited', $accredited)
                    ->with('totaltestcounts', $totaltestcounts)
                    ->with('specimencount', $specimencount)
                    ->with('percentagerate', $percentagerate)
                    ->with('error', $error)
                    ->withInput($request->all());
            }
        }
    ...
    */
    ...
    public function turnaroundTime(Request $request)
    {
        ...
        if ($request->has('filter')) {
            ...
            $databeyond = TestType::getTurnaroundBeyondlist($from, $to, $testTypeID);
            ...
            $databeyond = TestType::getTurnaroundBeyondlist($periodStart, $periodEnd);
        ...
        if ($request->has('word')) {
            ...
            $content = view('reports.tat.exportTAT')
                ...
                ->withInput($request->all());
            return Response::make($content, 200, $headers);
        } elseif ($request->has('beyond')) {
            $date = date("Ymdhi");
            $fileName = "testtype_tat_beyond_" . $date . ".xls";
            $headers = array(
                "Content-type" => "text/csv",
                "Content-Disposition" => "attachment;Filename=" . $fileName
            );
            $content = view('reports.tat.exportBeyondTAT')
                ->with('databeyond', $databeyond)
                ->withInput($request->all());
            return Response::make($content, 200, $headers);
        } else {
    ...
    public function turnaroundTimeBeyond(Request $request)
    {
        $from = $request->get('start');
        $to = $request->get('end');
        $today = date('Y-m-d');
        ...
        if ($request->has('filter')) {
            ...
            $months = json_decode(self::getMonths($from, $to));
            $data = TestType::getTurnaroundBeyondlist($from, $to, $testTypeID);
            $chart = self::getturnaroundTimeChart($request, $testTypeID);
        }
    ...
    public static function getturnaroundTimeChart(Request $request, $testTypeID = 0)
    {
        ...
        $options = '{
			...
			"title": {
				"text":"' . trans('messages.turnaround-timee') . '"
			},
			"subtitle": {
				"text":';
        if ($from == $to)
    ...
    public function infectionReport(Request $request)
    {
        ...
        if ($request->has('word')) {
            $date = date("Ymdhi");
            $fileName = "test_report_" . $date . ".xls";
            $headers = array(
                "Content-type" => "text/csv",
                "Content-Disposition" => "attachment;Filename=" . $fileName
            );
    ...
    public function userStatistics(Request $request)
    {
        ...
        switch ($selectedReport) {
            case '1':
                ...
                if ($request->has('word')) {
                    $date = date("Ymdhi");
                    $fileName = "Patient_register_" . $date . ".xls";
                    $headers = array(
                        "Content-type" => "text/csv",
                        "Content-Disposition" => "attachment;Filename=" . $fileName
                    );
            ...
            case '2':
                ...
                if ($request->has('word')) {
                    $date = date("Ymdhi");
                    $fileName = "specimen_register_log_" . $date . ".xls";
                    $headers = array(
                        "Content-type" => "text/csv",
                        "Content-Disposition" => "attachment;Filename=" . $fileName
                    );
            ...
            case '3':
                ...
                if ($request->has('word')) {
                    $date = date("Ymdhi");
                    $fileName = "test_register_log_" . $date . ".xls";
                    $headers = array(
                        "Content-type" => "text/csv",
                        "Content-Disposition" => "attachment;Filename=" . $fileName
                    );
    ...
    public function downloadMicrobiology(Request $request)
    {
        ...
        return Excel::download(new MicrobiologyExport($dateFrom, $dateTo), 'microdata.xlsx');
    }

    public function downloadMicrobiology22(Request $request)
    {
    ...
    public function tb_register(Request $request)
    {
        ...
        if ($request->has('word')) {
            $date = date("Ymdhi");
            $fileName = "tb_test_records_" . $date . ".xls";
            $headers = array(
                "Content-type" => "text/csv",
                "Content-Disposition" => "attachment;Filename=" . $fileName
            );
    ...
    public function downloadAlisRestrackExport(Request $request)
    {
        $dateFrom =  $request->get('date_from');
        $dateTo = $request->get('date_to');
        ...
        return Excel::download(new AlisRestrackExport($dateFrom, $dateTo), 'AlisRestrackData.xlsx');
    }

    public function exportAlisRestrackData()
    {
        $dateFrom =  date('Y-m-01');
        $dateTo = date('Y-m-d');

        // Load the view and pass it the tests
        return view('reports.alis_restrack')
            ->with('dateFrom', $dateFrom)
            ->with('dateTo', $dateTo);
    }
