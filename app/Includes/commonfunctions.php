...
function returnDrugSusceptibityResults($isolated_organism_id)
{
	$result = "SELECT ds.id,d.name,ds.zone_diameter,dsm.symbol,iso.test_id,iso.organism_id,o.name FROM drug_susceptibility ds
				join isolated_organisms iso ON ds.isolated_organism_id = iso.id
				join drug_susceptibility_measures dsm ON ds.drug_susceptibility_measure_id = dsm.id
				join drugs d ON ds.drug_id = d.id
				join organisms o ON o.id = iso.organism_id
				where iso.id = ".$isolated_organism_id;
	\Log::info($result);
